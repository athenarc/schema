...
class ROCrateModal
{
	public static function addModal($jobid)
	{
		...
		echo		 '<span class="col-md-5" >'.$form->field($model,'software_url')->label("")->textInput(['value'=>$image_url, 'disabled'=>$disabled]).'</span>
				</div>';
		...
		foreach ($fields as $field) 
		{
			...
			if ($field->field_type=='File')
			{
				...
				echo "<span class='col-md-5'>" . $form->field($model,"input[$field->name]")->label("")->textInput(['id'=>'field-' . $i, 'value'=>empty($field_to_url[$field->name])?'':$field_to_url[$field->name]]) . '</span>';
				echo "<span class='col-md-12 local-file'> Used file: ". $fields_folder . '' . $fields_file[$field->name]."  </span>";
				echo "</div>";
		...
		echo  '<div class="row body-row">
					...
					<span class="col-md-5">'. $form->field($model,'output')->label("")->textInput(['value'=>$model->output]).'</span>';
		echo	'</div>';
		echo  "<div class='row body-row'>
					<span class='col-md-7 field-row'> Publication DOI $publication_icon :</span> 
					<span class='col-md-5'>". $form->field($model,'publication')->label("")->textInput(['value'=>$model->publication])."</span>
				</div>";
		
		echo  "<div class='row body-row'>
					<span class='col-md-7'> Download experiment locally $download_icon :</span> 
